...
const useStripeAccount = () => {
    ...
    useEffect(() => {
        getStaffWalletMe();
        getStaffList({pageNo: currentPage, pageSize});
    }, []);
    ...
    const getStaffList = async ({pageNo, pageSize: requestedPageSize, tab}: any) => {
        const url =
        ...
        try {
            const res = await fetch(url, {
                ...
                body: JSON.stringify({page_no: pageNo, page_size: requestedPageSize})
            });
            if (!res.ok) {
