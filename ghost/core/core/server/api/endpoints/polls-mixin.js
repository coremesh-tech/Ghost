...
function buildProxyError(statusCode, body) {
    ...
    if (statusCode === 400) {
        return new errors.BadRequestError(errorOptions);
    }

    if (statusCode === 401) {
        return new errors.UnauthorizedError(errorOptions);
    }

    if (statusCode === 403) {
        return new errors.NoPermissionError(errorOptions);
    }

    if (statusCode === 404) {
        return new errors.NotFoundError(errorOptions);
    }

    if (statusCode === 409) {
        return new errors.ConflictError(errorOptions);
    }

    return new errors.InternalServerError(errorOptions);
}
